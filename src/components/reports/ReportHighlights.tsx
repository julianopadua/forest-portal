...
import type { ReportHighlight } from "@/lib/reports/types";
...
export default function ReportHighlights({ highlights }: { highlights: ReportHighlight[] }) {
  return (
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      {highlights.map((item) => {
        const pct = formatPct(item.pct_change);
        const pctTone =
            ...
            {(item.comparison_label || item.comparison_value !== null) && (
              <div className="mt-3 text-sm text-[color:var(--muted)]">
                ...
                <span className="font-medium text-[color:var(--foreground)]">
                  {formatValue(item.comparison_value)}
                </span>
              </div>
            )}
