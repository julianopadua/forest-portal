...
export default function Header() {
  ...
  // auto-open modal via query (?auth=signin|signup)
  useEffect(() => {
    const q = sp.get("auth");
        ...
        <div className="mx-auto max-w-6xl px-3 sm:px-4">
          <div className="mt-3 rounded-2xl border border-[color:var(--border)] bg-[color:var(--surface)] backdrop-blur-xl" style={{ boxShadow: "var(--shadow-float)" }}>
            ...
            <div className="flex items-center justify-between px-3 py-3 sm:px-4 md:hidden">
              <button onClick={() => setOpenMenu(true)} className="rounded-xl border border-[color:var(--border)] bg-[color:var(--surface-2)] px-3 py-2 hover:bg-[color:var(--surface-3)]">
                ☰
              </button>
              <Link href="/">
            ...
            <div className="hidden md:flex md:items-center md:gap-3 md:px-3 md:py-3 sm:px-4">
              <div className="flex shrink-0 items-center gap-3">
                <button onClick={() => setOpenMenu(true)} className="inline-flex rounded-xl border border-[color:var(--border)] bg-[color:var(--surface-2)] px-3 py-2 hover:bg-[color:var(--surface-3)]">
                  ☰
                </button>
                <Link href="/">
              ...
              <nav className="hidden md:flex flex-1 min-w-0 items-center justify-center gap-1">
                <Link className="rounded-xl px-2 py-2 text-xs text-[color:var(--muted)] hover:bg-[color:var(--surface-2)] lg:px-3 lg:text-sm" href={`/${openDataId}`}>{dict.marketing.sections.mission.title}</Link>
                <Link className="rounded-xl px-2 py-2 text-xs text-[color:var(--muted)] hover:bg-[color:var(--surface-2)] lg:px-3 lg:text-sm" href={`/${commoditiesId}`}>{dict.marketing.sections.programs.title}</Link>
                <Link className="rounded-xl px-2 py-2 text-xs text-[color:var(--muted)] hover:bg-[color:var(--surface-2)] lg:px-3 lg:text-sm" href={`/${reportsId}`}>{dict.marketing.sections.contents.title}</Link>
                <Link className="rounded-xl px-2 py-2 text-xs text-[color:var(--muted)] hover:bg-[color:var(--surface-2)] lg:px-3 lg:text-sm" href={`/${educationId}`}>{dict.marketing.sections.community.title}</Link>
              </nav>
